...
{
    private readonly Dictionary<Guid,GoodItemDao>  _goodItems = new();
    private readonly ReaderWriterLock _lock = new();
    private readonly int _timeout = 70;
    public Guid Add(GoodItemDao goodItem)
    {
        try
        {
    ...
    public GoodItemDao Find(Guid id)
    {
        try
        {
    ...
    public GoodItemDao UpdatePrice(Guid id, decimal newPrice)
    {
        try
        {
            _lock.AcquireWriterLock(_timeout);
